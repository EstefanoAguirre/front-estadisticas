...
class BtnExportProgramas extends Component{
    ...
    printDocument() {
        ...
        var pdf = function (graf, columns, rows, titulo, usuario) {
            ...
            const input = document.getElementById(graf);
                ...
                docTabla.output('save', 'ReporteProgramasTabla.pdf');//guardar pdf
            
        }

        var data = this.state.tableProgramasData;
        ...
        var columns = ["Programas", "Concepto", "Importe"];
    ...
    render() {
        return (<div>
            <div className="mb5">
                <button className="btn btn-warning btn-block" onClick={this.printDocument}><b>Imprimir programas</b></button>
            </div>
        </div>);
    }
}

export default BtnExportProgramas;
